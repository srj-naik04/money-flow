...
async function txConds(
  ...
  // Tenant scope first — every analytics query funnels its base filter through here.
...
export async function getAnalytics(
  ...
  const catRows = await db
    .select({
      ...
      amount: sql<number>`coalesce(sum(${transactions.grossAmount}),0)`.mapWith(
        Number,
      ),
    })
    .from(transactions)
  ...
  // Tenant scope first — the history rows must belong to the current user.
  const invConds: SQL[] = [eq(investmentValueHistory.userId, userId)];
  if (projectId && projectId !== "all") {
    invConds.push(eq(investments.projectId, projectId));
  } else if (!settings.includeArchivedInTotals) {
    const archivedInv = await archivedProjectIds();
    if (archivedInv.length) {
      const c = or(
        isNull(investments.projectId),
        notInArray(investments.projectId, archivedInv),
      );
      if (c) invConds.push(c);
    }
  }
  const history = await db
    ...
    .leftJoin(
      investments,
      and(
        eq(investmentValueHistory.investmentId, investments.id),
