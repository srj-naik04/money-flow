...
export function CommandPalette() {
  ...
  const hitGroups = [
    { key: "transactions", title: "Transactions", items: hits?.transactions ?? [] },
    { key: "projects", title: "Projects", items: hits?.projects ?? [] },
    { key: "subscriptions", title: "Subscriptions", items: hits?.subscriptions ?? [] },
    { key: "investments", title: "Investments", items: hits?.investments ?? [] },
          ...
          <>
            <CommandGroup heading="Quick add">
              {QUICK_ACTIONS.map((a) => {
                const Icon = a.icon;
                return (
                  <CommandItem key={a.type} onSelect={() => run(() => openQuickAdd(a.type))}>
            ...
            <CommandGroup heading="Filter project">
              <CommandItem onSelect={() => run(() => setActiveProject("all"))}>All Projects</CommandItem>
              {projects?.map((p) => (
                <CommandItem key={p.id} onSelect={() => run(() => setActiveProject(p.id))}>
          ...
          <>
            ...
            {hitGroups.map((g) => (
              <CommandGroup key={g.key} heading={g.title}>
                {g.items.map((hit) => (
                  <CommandItem key={hit.id} onSelect={() => run(() => router.push(hit.href))}>
                    <SearchIcon className="size-4 text-muted-foreground" />
                    <span className="flex-1 truncate">{hit.label}</span>
                    {hit.amount !== null ? (
                      <Money paise={hit.amount} className="text-xs text-muted-foreground" />
                    ) : hit.sublabel ? (
