...
import { Plus, MoreHorizontal, Pencil, Archive, Trash2, FolderKanban, ArchiveRestore } from "lucide-react";
...
import { useProjectsWithStats, useUpdateProject, useDeleteProject } from "@/hooks/use-projects";
...
export default function ProjectsPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          {data.map((p) => (
              ...
              >
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{p.txnCount} txns</span>
                </div>

                <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
                  <div>
                    <p className="text-xs text-muted-foreground">Income</p>
                    <Money paise={p.income} className="font-medium text-positive" compact />
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Expense</p>
                    <Money paise={p.expense} className="font-medium text-negative" compact />
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Net</p>
                    <Money paise={p.net} className="font-medium" colorBySign compact />
                  </div>
                </div>
              </div>
          ))}
      ...
      <ProjectFormModal open={formOpen} onOpenChange={setFormOpen} project={editing} />
