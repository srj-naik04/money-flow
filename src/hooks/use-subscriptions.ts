  ...
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      api.get<SubscriptionDTO[]>(
        `/api/subscriptions${projectQuery(projectId)}`,
        signal,
      ),
...
export function useUpcoming(
  ...
  return useQuery({
    ...
    queryFn: ({ signal }) => {
      ...
      return api.get<SubscriptionDTO[]>(
        `/api/subscriptions/upcoming?${p}`,
        signal,
      );
    },
    placeholderData: keepPreviousData,
...
export function useDeleteSubscription() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.del<{ id: string }>(`/api/subscriptions/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.all }),
  });
}
